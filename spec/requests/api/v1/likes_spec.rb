...
RSpec.describe "Api::V1::Likes", type: :request do
  describe "POST /api/v1/posts/:post_id/likes" do
    ...
    it "creates a like on another user's visible post" do
      expect do
        post api_v1_post_likes_path(posts(:one).id), headers: auth_headers(users(:three)), as: :json
      end.to change(Like, :count).by(1)
    ...
    it "rejects a like on a post the user cannot see" do
      expect do
        post api_v1_post_likes_path(posts(:two).id), headers: auth_headers(users(:three)), as: :json
      end.not_to change(Like, :count)
  ...
  describe "resource actions" do
    ...
    it "rejects deleting a like that belongs to someone else" do
      expect do
        delete api_v1_post_like_path(posts(:one).id, likes(:one).id), headers: auth_headers(users(:one))
      end.not_to change(Like, :count)
