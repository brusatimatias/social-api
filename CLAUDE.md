...
- Auth is stateless JWT: `issue_token`/`decoded_token` encode/decode `{ sub: user.id, exp: ... }` with `Rails.application.secret_key_base`. `current_user` is resolved from the `Authorization: Bearer <token>` header per-request; there are no sessions.
